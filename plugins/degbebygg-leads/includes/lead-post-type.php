...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'degbebygg-leads' ),
					'singular_name' => __( 'Lead', 'degbebygg-leads' ),
					'edit_item'     => __( 'Förfrågan', 'degbebygg-leads' ),
					'search_items'  => __( 'Search leads', 'degbebygg-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'degbebygg-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'degbebygg-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'degbebygg-leads' ),
			'npl_phone'    => __( 'Phone', 'degbebygg-leads' ),
			'npl_service'  => __( 'Needs', 'degbebygg-leads' ),
			'npl_timeline' => __( 'When', 'degbebygg-leads' ),
			'npl_zip'      => __( 'ZIP', 'degbebygg-leads' ),
			'npl_source'   => __( 'Source', 'degbebygg-leads' ),
			'npl_status'   => __( 'Status', 'degbebygg-leads' ),
			'date'         => __( 'Received', 'degbebygg-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'degbebygg-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'degbebygg-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'degbebygg-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'degbebygg-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'degbebygg-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'degbebygg-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'degbebygg-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'degbebygg-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'degbebygg-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'degbebygg-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'degbebygg-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'degbebygg-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';

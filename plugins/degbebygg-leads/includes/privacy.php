...
add_filter(
	...
	function ( $exporters ) {
		$exporters['degbebygg-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'degbebygg-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'degbebygg-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'degbebygg-leads',
						'group_label' => __( 'Quote requests', 'degbebygg-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['degbebygg-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'degbebygg-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {

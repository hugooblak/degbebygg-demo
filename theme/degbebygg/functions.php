<?php
/**
 * Degbe Bygg theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Degbe Bygg Leads" plugin, so leads survive a theme change.
 *
 * @package Degbe Bygg
 */
 ...
 * Patterns read these through de_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'degbebygg', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'degbebygg' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'degbebygg' ) ) );

		register_block_pattern_category( 'degbebygg-sections', array( 'label' => __( 'Degbe Bygg sections', 'degbebygg' ) ) );
		register_block_pattern_category( 'degbebygg-pages', array( 'label' => __( 'Degbe Bygg page layouts', 'degbebygg' ) ) );
	}
);

 ...
 * or photo. They call de_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Degbe Bygg.
 *
 * @package Degbe Bygg
 */
 ...
 * @return string
 */
function de_sample_note() {
	return de_lead( 'ar_riktig' ) ? '' : ' <span class="de-sample">(platshållare)</span>';
}
 ...
 * @return string File name for de_lead_img().
 */
function de_lead_bild( $i ) {
	$bilder = (array) de_lead( 'bilder', array() );
 ...
 * @return string
 */
function de_ring_label() {
	$namn = de_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, de_lead( 'telefon' ) ) : 'Ring ' . de_lead( 'telefon' );
}
